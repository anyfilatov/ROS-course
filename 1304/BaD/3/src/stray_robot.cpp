...
int main(int argc, char** argv) {
    ...
    while (ros::ok()) {
        ...
        stray_robot_transform.setRotation(tf::Quaternion(robot.pose.orientation.x, robot.pose.orientation.y, robot.pose.orientation.z, robot.pose.orientation.w));
        ...
        if (state == State::Chase) {
            ...
            if (distance <= epsilon) {
                ...
                currentLinearSpeed = linearSpeed * distance;
                currentAngularSpeed = angularSpeed * atan2(dy, dx);
                if (currentLinearSpeed > maxSpeed) {
                    currentLinearSpeed = maxSpeed;
                }
                robotX += currentLinearSpeed * std::cos(currentAngularSpeed);
                robotY += currentLinearSpeed * std::sin(currentAngularSpeed);
            }
