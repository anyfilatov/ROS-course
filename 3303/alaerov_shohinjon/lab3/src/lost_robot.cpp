...
int main(int argc, char** argv)
{
	...
	double stepY = GenerateStep();
		...
		if(Distance(transform.getOrigin().x(), transform.getOrigin().y(), 
				        marker.pose.position.x, marker.pose.position.y)<10)
			break;
      		rate.sleep();
		stepNumber++;
    	}
		...
		double angle = atan2(transform.getOrigin().y(), transform.getOrigin().x());
    		marker.pose.position.x+=ExitStep*cos(angle);
		marker.pose.position.y+=ExitStep*sin(angle);
    		publisher.publish(marker);
 		rate.sleep();
    	}
}
