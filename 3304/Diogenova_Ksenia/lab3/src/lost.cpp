...
void sendTransform(float x, float y)
{
    ...
    br.sendTransform(tf::StampedTransform(transform, ros::Time(4), "/world", "/lost"));
}
...
tf::StampedTransform listenTransform()
{
    ...
    listener.waitForTransform("/world", "/rescue", ros::Time(4), ros::Duration(1.0));
    listener.lookupTransform("/world", "/rescue", ros::Time(4), transform);
