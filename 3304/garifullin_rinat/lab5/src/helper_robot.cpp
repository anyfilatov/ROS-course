...
int main(int argc, char *argv[])
{
	...
	while(true)
		...
		oldAngle=angle;
        	...
        	publisher.publish(msg);
                if(Distance(transform.getOrigin().getX(), transform.getOrigin().getY(), 
			msg.pose.position.x, msg.pose.position.y)<2)
                {
			break;
                }
        	rate.sleep();
	...
	{	
		...
		else
		{
    		...
    		msg.pose.orientation.w=cos(angle/2);
        	publisher.publish(msg);
		if(Distance(ExitX, ExitY, msg.pose.position.x, msg.pose.position.y)<2)
                {
			break;
                }
        	rate.sleep();
