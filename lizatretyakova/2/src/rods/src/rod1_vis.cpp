...
int main( int argc, char** argv )
{
    ...
    if(argc != 3) {
        ROS_ERROR("not enough arguments for markers!!!");
        return -1;
    }
    ROS_INFO("%d %s %s\n", argc, argv[1], argv[2]);
    std::string frame(argv[1]);
    const char* cstr = frame.c_str();
    ROS_INFO("%s\n", cstr);
    std::string id_str(argv[2]);
    std::istringstream id_str_in(id_str);
    int id;
    id_str_in >> id;
    ...
    while (ros::ok()) {
        ...
        // Set the marker type.  
        // Initially this is CUBE, and cycles between that 
        ...
        // This is a full 6DOF pose relative to the frame/time 
